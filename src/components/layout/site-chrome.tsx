...
export function SiteChrome() {
  ...
  return (
    <header 
      className={`pointer-events-none fixed inset-x-0 top-0 z-30 pt-6 transition-opacity duration-700 ease-in-out ${
    ...
    >
      <Container className="flex h-14 items-center justify-between">
        ...
        <div className={`flex items-center gap-6 transition-all duration-700 delay-200 ${isVisible ? "pointer-events-auto transform translate-y-0" : "pointer-events-none transform -translate-y-4"}`}>
          <p className="hidden text-[10px] font-medium uppercase tracking-[0.25em] text-white/50 mix-blend-screen md:block">
            Shillong — Guwahati
          </p>
          <a href="/resume" target="_blank" className="text-[9px] md:text-[10px] font-medium uppercase tracking-[0.2em] text-white/90 border border-white/20 px-4 py-2 rounded-full hover:bg-white hover:text-black transition-colors duration-300 backdrop-blur-sm bg-black/20">
            Download Resume
          </a>
        </div>
      </Container>
